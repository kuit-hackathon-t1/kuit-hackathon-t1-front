...
const shapeClass: Record<InsectShape, string> = {
...
export default function SpecimenImage({ imageUrl, shape, status, alt = "채집 이미지" }: SpecimenImageProps) {
  return (
    <div
      className={cn(
        "aspect-square w-full overflow-hidden rounded-lg border bg-neutral-100",
        status === "FAILED" ? "border-dashed border-neutral-400 opacity-60 grayscale" : "border-neutral-200",
      )}
    >
      {imageUrl ? (
        <img className={cn("h-full w-full object-cover", shapeClass[shape])} src={imageUrl} alt={alt} />
      ) : (
