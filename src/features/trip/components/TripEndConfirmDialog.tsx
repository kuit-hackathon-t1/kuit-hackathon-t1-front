...
export default function TripEndConfirmDialog({ open, isPending = false, onClose, onConfirm }: TripEndConfirmDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[80] mx-auto flex w-full max-w-[430px] items-center justify-center bg-black/35 px-5 py-6">
      <Card className="w-full max-w-[360px] rounded-[32px] border-gray-200 bg-white p-6 text-center shadow-card">
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-primary-soft">
          <img className="h-8 w-8" src={warningIcon} alt="" aria-hidden="true" />
        </div>
